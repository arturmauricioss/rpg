document.addEventListener("DOMContentLoaded", () => {
  ...
  const attributeInputs = [
    {
      ...
      button: document.getElementById("roll-strength"),
      rollsSpan: document.getElementById("strength-rolls"),
      rolls: [],
    },
    {
      id: "dexterity",
      input: document.getElementById("dexterity"),
      button: document.getElementById("roll-dexterity"),
      rollsSpan: document.getElementById("dexterity-rolls"),
      rolls: [],
    },
    {
      id: "constitution",
      input: document.getElementById("constitution"),
      button: document.getElementById("roll-constitution"),
      rollsSpan: document.getElementById("constitution-rolls"),
      rolls: [],
    },
    {
      id: "intelligence",
      input: document.getElementById("intelligence"),
      button: document.getElementById("roll-intelligence"),
      rollsSpan: document.getElementById("intelligence-rolls"),
      rolls: [],
    },
    {
      id: "wisdom",
      input: document.getElementById("wisdom"),
      button: document.getElementById("roll-wisdom"),
      rollsSpan: document.getElementById("wisdom-rolls"),
      rolls: [],
    },
    {
      id: "charisma",
      input: document.getElementById("charisma"),
      button: document.getElementById("roll-charisma"),
      rollsSpan: document.getElementById("charisma-rolls"),
      rolls: [],
    },
  ];
  ...
  function applyModifiersToAttributes(
    attributes,
    race,
    characterClass,
    gender,
    ...
    if (classModifiers[characterClass]) {
      for (let attribute in classModifiers[characterClass]) {
        modifiedAttributes[attribute] +=
          classModifiers[characterClass][attribute];
      }
    }
    ...
    if (genderModifiers[gender]) {
      for (let attribute in genderModifiers[gender]) {
        modifiedAttributes[attribute] +=
          genderModifiers[gender][attribute] || 0;
      }
    }
  ...
  const characterSheetContainer = document.getElementById(
  ...
  function generateCharacter(event) {
    ...
    const selectedRaceData = raceOptions.find(
    ...
    const allAttributesFilled = attributeInputs.every(
    ...
    const modifiedAttributes = applyModifiersToAttributes(
      attributes,
   ...
   Nome: ${characterName}
   Raça: ${
     raceOptions.find((race) => race.value === selectedRace)?.label ||
     selectedRace
   }
   Classe: ${
     classOptions.find((cls) => cls.value === selectedClass)?.label ||
     selectedClass
   }
...
`;
    ...
    characterSheet.innerHTML = `
        ...
        <div style="flex: 1 1 120px; margin: 10px; background: #f4f4f4; padding: 15px; border-radius: 5px;">
            ...
            <p>Alinhamento: ${
              alignmentTranslations[selectedAlignment] || selectedAlignment
            }</p>
        </div>
        <div style="flex: 1 1 120px; margin: 10px; background: #f4f4f4; padding: 15px; border-radius: 5px;">
  ...
  function updateCreateButtonState() {
    ...
    const attributesFilled = attributeInputs.every(
      (attr) => attr.input.value !== ""
    );

    // Verifica se todos os botões de rolagem estão desabilitados (atributos rolados)
    const allButtonsDisabled = attributeInputs.every(
      (attr) => attr.button.disabled
    );
  ...
  toStep2Btn.addEventListener("click", () => {
    ...
    if (raceSelect.value === "") {
      customAlert.textContent =
        "Preencha todas as opções e selecione uma raça antes de prosseguir!";
